...
def fetch_cves_multisource(service_name: str) -> List[Dict[str, Any]]:
    ...
    # Fallback to CIRCL if NVD returned nothing or few results
    if len(cves) < 3:
...
def upsert_cves(
    ...
    for cve in cves:
        ...
        if year is None or year < start_year or year > end_year:
            continue
        # Enrich using CVE detail endpoint when available
        details = fetch_cve_details(cve.get("cve_id")) if cve.get("cve_id") else {}
        affected = cve.get("affected_configurations") or details.get("affected_configurations", "")
        published = cve.get("published_date") or details.get("published_date", "")
        refs = cve.get("reference_links", []) or details.get("reference_links", [])
        pocs = cve.get("poc_references", []) or details.get("poc_references", [])
